...
{
    public class GCNTextureDialog : Window
    {
        //Name of window
        ...
        /// </summary>
        public List<GCNImportedTexture> Textures = new List<GCNImportedTexture>();
        ...
        private void OnLoad() {
            ...
            DecodedTexture.Bind();
            DecodedTexture.SetFilter(OpenTK.Graphics.OpenGL.TextureMinFilter.Nearest, OpenTK.Graphics.OpenGL.TextureMagFilter.Nearest);
            DecodedTexture.Unbind();
        }

        public GCNTextureDialog()
        {
        ...
        /// </summary>
        public GCNImportedTexture AddTexture(string fileName) {
            ...
            var tex = new GCNImportedTexture(fileName);
            Textures.Add(tex);

            //Default to ETC1 (best compression size) but check for alpha and other bits 
            tex.Format = tex.TryDetectFormat(Decode_Gamecube.TextureFormats.CMPR);
        ...
        /// The settings are returned and can be configured per file.
        /// </summary>
        public GCNImportedTexture AddTexture(string name, byte[] rgbaData, uint width, uint height, uint mipCount, Decode_Gamecube.TextureFormats format)
        {
            var tex = new GCNImportedTexture(name, rgbaData, width, height, mipCount, format);
            Textures.Add(tex);
            return tex;
        }
        ...
        private void DrawProperties()
        {
            ...
            if (ImGui.BeginChild("##texture_dlg_properties", wndsize))
            {
                ...
                {
                    foreach (Decode_Gamecube.TextureFormats format in Enum.GetValues(typeof(Decode_Gamecube.TextureFormats)))
                    {
                ...
                if (texture.Format == Decode_Gamecube.TextureFormats.C4 ||
                    texture.Format == Decode_Gamecube.TextureFormats.C8)
                {
                    if (ImGui.BeginCombo("Palette Format", texture.PaletteFormat.ToString()))
                    {
                        foreach (Decode_Gamecube.PaletteFormats format in Enum.GetValues(typeof(Decode_Gamecube.PaletteFormats)))
                        {
                            bool isSelected = format == texture.PaletteFormat;
                            if (ImGui.Selectable(format.ToString()))
                            {
                                //Disable current display
                                DecodedTexture.Reload(1, 1, new byte[4]);
                                ...
                                foreach (var selection in SelectedIndices)
                                {
                                    Textures[selection].PaletteFormat = format;
                                    //Re encode format
                                    Textures[selection].Encoded = false;
                                }
                                ReloadImageDisplay();
